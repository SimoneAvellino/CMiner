...
class DirectedNodeExtensionManager(NodeExtensionManager):
    ...
    def add(self, pattern_node_id, target_node_id, neigh_target_node_id, db_graph, _map):
        ...
        for label in self.memoization[(target_node_id, neigh_target_node_id)]:
            edge_labels.append(DirectedNodeExtensionManager.orientation_code(label, True))
        for label in self.memoization[(neigh_target_node_id, target_node_id)]:
            edge_labels.append(DirectedNodeExtensionManager.orientation_code(label, False))
            ...
            for e in ext:
                frequent_extensions.append(
                    NodeExtension(
                        ...
                        DirectedExtension(e.out_edge_labels, e.in_edge_labels, e.location))
                    )
            ...
            for e in ext:
                frequent_extensions.append(
                    NodeExtension(
                        pattern_node_id, 
                        node_labels_code.split(" "), 
                        UndirectedExtension(e.out_edge_labels, e.location)) # NOTE: i use only out_edge_labels because the in_edge_labels are not used in undirected graphs
                    )
        return frequent_extensions
