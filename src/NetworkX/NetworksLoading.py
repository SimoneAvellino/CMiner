...
class NetworksLoading:
    def __init__(self, file_type, metadata):
        self.Networks = {}
    ...
    def _graph_type(is_directed, is_multigraph):
        ...
        """
        if is_directed and is_multigraph     : return MultiDiGraph()
        if is_directed and not is_multigraph : return DiGraph()
        if not is_directed and is_multigraph : return MultiGraph()
        return Graph()
    ...
    def _multiple_rows(components, nodes_dict, sep):
        ...
        :return: None
        """
        if components[1] not in nodes_dict: nodes_dict[components[1]] = []
        nodes_dict[components[1]].append(components[2])
    ...
    def _load_csv_file(self, metadata):
        ...
        for network in listdir(metadata[ND]):
            ...
            for file in listdir(nodes_path):
                ...
                file_nodes[metadata[LC]] = file_nodes[metadata[LC]].str.split(metadata[SL])
            ...
            for file in listdir(edges_path):
                ...
                file_edges   = [(row.pop(src_dst_cols[0]), row.pop(src_dst_cols[1]), row) for row in file_edges]
