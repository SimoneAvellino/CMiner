from .BitMatrix import (
    BitMatrixStrategy2,
    QueryBitMatrixOptimized,
    TargetBitMatrixOptimized,
)
from .BreakingConditions import BreakingConditionsEdges, BreakingConditionsNodes
from .CompatibilityDomain import CompatibilityDomainWithDictionary
...
class Mapping:
    ...
    def __init__(
        self, node_mapping=None, edge_mapping=None, extended_mapping: "Mapping" = None
    ):
    ...
    def get_mapped_graph(self, target_graph: DirectedMultiGraph) -> DirectedMultiGraph:
        mapped_graph = DirectedMultiGraph()
        node_mapping = self._retrieve_node_mapping()
        edge_mapping = self._retrieve_edge_mapping()
        for _, target_node in node_mapping.items():
            mapped_graph.add_node(
                int(target_node), labels=target_graph.get_node_labels(target_node)
            )
        for _, target_edge in edge_mapping.items():
            for label in target_graph.get_edge_labels(target_edge[0], target_edge[1]):
                mapped_graph.add_edge(
                    int(target_edge[0]), int(target_edge[1]), type=label
                )
        return mapped_graph
    ...
    def get_node_mapping_str(self):
        ...
        node_mapping_str += " ".join(
            [f"{k}->{v}" for k, v in self.node_mapping.items()]
        )
    ...
    def get_edge_mapping_str(self):
        ...
        if self.extended_mapping is not None:
            ...
            edge_mapping_str += previous_edge_mapping_str + (
                ", " if len(previous_edge_mapping_str) > 0 else ""
            )
        edge_mapping_str += " ".join(
            [f"{k}->{v}" for k, v in self.edge_mapping.items()]
        )
    ...
    def reset_edge_mapping(self, edge_mapping=None):
        ...
        """
        edge_mapping = (
            edge_mapping if edge_mapping is not None else self._retrieve_edge_mapping()
        )
...
class MultiGraphMatch:
    ...
    def match(
        ...
        if len(query_node_labels) > 0 and not all(
            label in target_node_labels for label in query_node_labels
        ):
            return []
        if len(query_edge_labels) > 0 and not all(
            label in target_edge_labels for label in query_edge_labels
        ):
            return []
        ...
        while i >= 0:
            if forceBack or self.cand_index[query_edge] >= len(self.cand[query_edge]):
                ...
                if all(
                    self.g[q_e] is None
                    for q_e in self.query.get_all_edges()
                    if q_e[0] == q_i or q_e[1] == q_i
                ):
                    self.f[q_i] = None
                ...
                t_i, t_j, key = target_edge = self.cand[query_edge][
                    self.cand_index[query_edge]
                ]
                # check if the target edge has not been already mapped to another query edge
                if all(
                    self.g[q_e] != target_edge
                    for q_e in self.query.get_all_edges()
                    if self.g[q_e] is not None
                ):
                    ...
                    if i == len(self.query.get_all_edges()) - 1:
                        ...
                        self.solutions.append(
                            Mapping(
                                node_mapping=self.f.copy(), edge_mapping=self.g.copy()
                            )
                        )
                    else:
        ...
        if self.f[q_i] is None and self.f[q_j] is None:
                ...
                for target_key in self.target.edges_keys((t_i, t_j)):
                    ...
                    if (
                        (
                            ...
                            not self.query.edge_has_label(query_edge)
                            or
                            # check if the edge labels are the same
                            self.target.get_edge_label(target_edge)
                            == self.query.get_edge_label(query_edge)
                        )
                        and
                        # check if the target edge source node contains the same attributes as the query edge source node
                        self.target.node_contains_attributes(
                            t_i, self.query.get_node_attributes(q_i)
                        )
                        and
                        # check if the target edge destination node contains the same attributes as the query edge destination node
                        self.target.node_contains_attributes(
                            t_j, self.query.get_node_attributes(q_j)
                        )
                        and
                        # check if the target edge contains the same attributes as the query edge
                        self.target.edge_contains_attributes(
                            target_edge, self.query.get_edge_attributes(query_edge)
                        )
                    ):
            ...
            for target_key in self.target.edges_keys((self.f[q_i], self.f[q_j])):
                ...
                if (
                    ...
                    self.target.get_edge_label(target_edge)
                    == self.query.get_edge_label(query_edge)
                ) and self.target.edge_contains_attributes(
                    target_edge, self.query.get_edge_attributes(query_edge)
                ):
                ...
                if t_i == self.f[q_i]:
                    for target_key in self.target.edges_keys((t_i, t_j)):
                        ...
                        if (
                            ...
                            and self.target.node_contains_attributes(
                                t_j, self.query.get_node_attributes(q_j)
                            )
                            and self.target.edge_contains_attributes(
                                target_edge, self.query.get_edge_attributes(query_edge)
                    ...
                    for target_key in self.target.edges_keys((t_i, t_j)):
                        ...
                        if (
                            (
                                # if query edge label is not specified it means that any edge label is accepted
                                not self.query.edge_has_label(query_edge)
                                or self.target.get_edge_label(target_edge)
                                == self.query.get_edge_label(query_edge)
                            )
                            and self.target.node_contains_attributes(
                                t_i, self.query.get_node_attributes(q_i)
